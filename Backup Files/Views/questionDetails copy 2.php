...
<div id="question-details">
    ...
    fetch(`<?= site_url('questions/vote_question/') ?>${questionId}/${direction}`, {
        method: 'POST',
        // If you use CSRF protection, make sure to pass the token here
        // headers: {'X-CSRF-TOKEN': 'your-csrf-token'}
    })
    .then(response => response.json())
    .then(data => {
        if(data.status === 'success') {
            // Assuming you have a span with id like question-vote-count-{questionId}
            alert(data.message);
        } else {
            alert(data.message);
        }
...
function voteAnswer(answerId, direction) {
    ...
    .then(data => {
        if(data.status === 'success') {
            // Assuming you have a span with id like answer-vote-count-{answerId}
            alert(data.message);
        } else {
            alert(data.message);
        }
